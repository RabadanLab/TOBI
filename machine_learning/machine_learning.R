...
#library("session", lib.loc="~/Library/R/3.1/library")#HARD CODED HERE?
...
if(vcftype == "TCGA"){
  param = c("Y", "case", "chrom", "pos", "id", "ref", "alt", "gene_name", "dp_2", "effect", #cjm, changed dp to dp_2 for t only
            "amino_acid_change", "warnings", "id2", "qual", "freq", "fa_1", "fa_2", "amino_acid_length", #"fa_1", "fa_2" cjm
            "gene_freq", "gene_length", "tot_cosm_samp", "num_mut_gene", "case_mut_gene",
            "effect_impact", "cosmic_nsamp", "pval_som", "dbNSFP_1000Gp1", #cjm, removed mq
...
dim(testing)
print(range(testing$dbNSFP_CADD_phred))

################################################################################
# Formula
my_formula = Y ~ freq + pval_som + 
  gene_freq + amino_acid_length +
  num_mut_gene + case_mut_gene + #id2 + #160129 cjm removed id2 
  cosmic_nsamp + tot_cosm_samp + dbNSFP_CADD_phred +
...
for (i in 1:4){ 
  ...
  print(perf_round[[i]] <- myPerf_y(testing))
  fscor_vec[i] <- perf_round[[i]][3] ##storing+printing myPerf; saving fscore
  mydata = testing
