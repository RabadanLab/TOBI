...
GBM_Recur_Mutations = c("PIK3R1", "PTEN", "PIK3CA", "TP53", "EGFR", "IDH1", 
                        "BRAF", "RB1", "NF1", "PDGFRA", "LRP2", "PPP2R3A", "FAT2",
                        "GPR116", "RIMS2", "NOS1", "PIK3C2B", "MDM4", "MYCN", 
                        "KIT", "DDIT3", "TSHZ2", "LRP1B", "CTNND2", "HCN1", "PKHD1",
                        "TEK", "PCNX", "HERC2", "LZTR1", "BCOR", "ATRX", "PCDH11X", 
                        "CDK4", "CDKN2A")
...
x = foreach(i=1:dim(genes)[1], .combine='rbind') %dopar% {
  ...
  y[19] = length(unique(mt$case[mt$gene_name == genes$name[i] & mt$pred == "som"]))
  y[20] = length(unique(mt$case[mt$gene_name == genes$name[i] & mt$Y == "som"]))
...
names(genes) = c('name', 'tot_num_mut', 'max_num_mut_case', 'num_recur', 'num_cases', 
                 'cosmic_nsamp', 'non_syn', 'stop_gained', 'syn', 'gene_freq', 
                 'amino_acid_length', 'effect_impact', 'MutationAssessor', 
                 'MutationTaster', 'per_mut_cosmic', 'per_mut_dbsnp', 'per_mut_none', 
                 'per_mut_som', 'per_mut_non_som', 'pred_som_cases', 
                 'act_som_cases', 'driver_gene')
