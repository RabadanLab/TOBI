driver_mutations = function(mydata, modboost, output_plot = TRUE) {
  ...
  GBM_Recur_Mutations = GBM_Recur_Mutations[GBM_Recur_Mutations %in% unique(mydata$gene_name)]
  ...
  if (output_plot) {
    ...
    q = qplot(x = gene, y = value, fill=variable,
              ...
              position="dodge")
    q = q + theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust = 1), 
